...
for sl in range(noSlices) : #range 12
    ...
    for i in range (nPhases):
        ...
        for j in range (nCoils):
            coil_corr[sl-10,j,i,:,:] = np.multiply(np.squeeze(ave_gridded_data[j,:,:]), np.conjugate(np.squeeze(coil_sensitivities[:,:,j,0])))
...
sio.savemat('meas_MID00573_FID48984_rest_stack_sl_10_11.mat',{'img_data':sum_coil}) #you can save as many arrays as you want
plt.show()
