...
contents = sio.loadmat('/home/david/shared/BART/data/kspace_rawData_exercise/frames_25/meas_MID00515_FID66434_7TH_BEAT_physics_radial_2DSAX.mat') #CHANGE
raw_data = contents['raw_data'] #shape (384, 13, 25, 34)
raw_data_shape = np.shape(raw_data)
print('raw_data_shape',raw_data_shape)
...
""" this is all making the trajectory from the loaded file
dimensions  = 3
...
sliceData = raw_data[:,:,:,:] #[384, 13, 40, 26, 12] , why do we have the 13 in here? 
...
for i in range (nPhases):
    ...
    for j in range (nCoils):
        coil_corr[0,j,i,:,:] = np.multiply(np.squeeze(ave_gridded_data[j,:,:]), np.conjugate(np.squeeze(coil_sensitivities[:,:,j,0])))
