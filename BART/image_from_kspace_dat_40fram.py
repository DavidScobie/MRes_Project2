...
contents = sio.loadmat('/home/david/shared/BART/data/frame_40_date_16_11_21_1_slice/raw_data/meas_MID00502_FID68284_radial_2DSAX_THISoNE_7TH.mat')  #17/11/21

raw_data = contents['raw_data'] #shape (384, 13, 40, 24)
...
trajectory = tfmr.radial_trajectory(192, views=13, phases=40, ordering='tiny_half', angle_range='full', readout_os=2.0) #(40, 13, 384, 2) 
trajectory = np.transpose(trajectory, [3, 2, 1, 0]) #(2,384,13,40)
dimensions = 2
...
for sl in range(noSlices) : 
    ...
    sliceData = raw_data[:,:,:,:] #[384, 13, 40, 24] , why do we have the 13 in here? 
    ...
    dataCS_perm = tf.transpose(dataCS_3124, perm=[3,0,2,1]) #(26,1,520,384)
    ...
    trajSc = tf.repeat(trajSc, repeats = nCoils, axis = 0)
    #scale from -pi to pi
    ...
    radial_weights = tfmr.radial_density(192, views=13*40, phases=1, ordering='tiny_half', angle_range='full', readout_os=2.0)
    ...
    radial_weights = tf.repeat(radial_weights, repeats = nCoils, axis = 0) #(26,199680)
    ...
    for i in range (nPhases):
        ...
        first_time_data = tf.reshape(first_time_data, [nCoils , -1]) #(26,4992) to enter to nufft yes

        #weights = tfmr.estimate_density(trajSc, (192,192))

        radial_weights = tfmr.radial_density(192, views=13, phases=1, ordering='tiny_half', angle_range='full', readout_os=2.0) #(1,1,4992)
        ...
        radial_weights = tf.repeat(radial_weights, repeats = nCoils, axis = 0) #(26,4992)
        ...
        for j in range (nCoils):
            coil_corr[sl-starting_slice,j,i,:,:] = np.multiply(np.squeeze(ave_gridded_data[j,:,:]), np.conjugate(np.squeeze(coil_sensitivities[:,:,j])))
...
# plt.figure(201)
# plt.imshow(abs(coil_corr[0,1,0,:,:])) #show slice 0, coil 1, 0th frame
# print('coil_corr',coil_corr.dtype)
